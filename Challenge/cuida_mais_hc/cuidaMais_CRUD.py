...
########################################Sprint 4#############################################
#encoding="utf-8" usado para aceitar caracteres especiais como acentos no arquivo json que esta sendo criado 
#ensure_ascii=False usado para aceitar caracteres especiais como acentos no arquivo json que esta sendo criado tambem
# indent=4 usado para formatar o json com identação de 4 espaços, deixando mais legível
#default=str usado para converter objetos que não são serializáveis por padrão em strings, como datas que estavam dando erro antes pois estavam vindo como tipo date do banco de dados


#pasta = "./cuida_mais_hc" 
#os.makedirs(pasta, exist_ok=True)
#arquivo = os.path.join(pasta, nm_arquivo)
#Essa parte do código acima cria a pasta caso ela não exista e junta o nome do arquivo com o caminho da pasta para salvar o arquivo json na pasta correta
#Aqui salvamos um arquivo JSON na pasta que está o codigo de execução, nessa pasta possui o arquiv rodar_api, que lê o arquvio JSON e cria um endpoint que passamos o id do paciente, e retorna uma API com os dados do paciente

def deseja_salvar_json():
    """
    DESCRIÇÃO
        Exibe uma pergunta para o usuário se deseja salvar os dados em um arquivo JSON.
    PARÂMETROS
        Essa função não recebe parâmetros.
    RETORNO
        Retorna resposta, que é True se o usuário desejar salvar os dados em um arquivo JSON, ou False caso contrário.
    """
    resposta = input("Deseja salvar os dados em um arquivo JSON? (sim/não): ")
    if resposta.upper() == 'SIM' or resposta.upper() == 'S' or resposta.upper() == 'YES' or resposta.upper() == 'Y' or resposta.upper() == 'SI':
        return True 
    else:
        return False

def salvar_json_paciente(id_paciente, nm_paciente, cpf_paciente, dt_nascimento_paciente, ddd, nr_telefone, nm_logradouro, nr_logradouro, nm_cidade, sg_estado):
    nm_arquivo = f"paciente_{id_paciente}.json"
    try:
        data_formatada = datetime.strptime(str(dt_nascimento_paciente), "%Y-%m-%d %H:%M:%S").strftime("%d/%m/%Y")
    except ValueError:
        try:
            data_formatada = datetime.strptime(str(dt_nascimento_paciente), "%Y-%m-%d").strftime("%d/%m/%Y")
        except ValueError:
            data_formatada = str(dt_nascimento_paciente) 
    paciente = {
        "id_paciente": id_paciente,
        "nm_paciente": nm_paciente,
        "cpf_paciente": str(cpf_paciente),
        "dt_nascimento_paciente": data_formatada,
        "ddd": ddd,
        "nr_telefone": nr_telefone,
        "nm_logradouro": nm_logradouro,
        "nr_logradouro": nr_logradouro,
    ...
    try:
        ...
        with open(arquivo, 'w', encoding='utf-8') as arquivo_json:
            json.dump(paciente, arquivo_json, ensure_ascii=False, indent=4, default=str)
        print(f'\n[SUCESSO] Dados escritos com sucesso no {nm_arquivo}')
    except Exception as e:
        print(f'\n[Erro] Ocorreu um erro ao escrever no arquivo: {e}')

def salvar_json_consulta(id_consulta, id_paciente_consulta, id_medico_consulta, data_consulta, descricao):
    nm_arquivo = f"consulta_{id_consulta}.json"
    try:
        data_formatada = datetime.strptime(str(data_consulta), "%Y-%m-%d %H:%M:%S").strftime("%d/%m/%Y")
    except ValueError:
        try:
            data_formatada = datetime.strptime(str(data_consulta), "%Y-%m-%d").strftime("%d/%m/%Y")
        except ValueError:
            data_formatada = str(data_consulta) 
    consulta = {
        "id_consulta": id_consulta,
    ...
    try:
        ...
        arquivo = os.path.join(pasta, nm_arquivo)
        with open(arquivo, 'w', encoding='utf-8') as arquivo_json:
            json.dump(consulta, arquivo_json, ensure_ascii=False, indent=4, default=str)
        print(f'\n[SUCESSO] Dados escritos com sucesso no {nm_arquivo}')
    except Exception as e:
        print(f'\n[Erro] Ocorreu um erro ao escrever no arquivo: {e}')

def salvar_json_medico(id_medico, nm_medico, cpf_medico, dt_nascimento_medico, especialidade, crm):
    nm_arquivo = f"medico_{id_medico}.json"
    try:
        data_formatada = datetime.strptime(str(dt_nascimento_medico), "%Y-%m-%d %H:%M:%S").strftime("%d/%m/%Y")
    except ValueError:
        try:
            data_formatada = datetime.strptime(str(dt_nascimento_medico), "%Y-%m-%d").strftime("%d/%m/%Y")
        except ValueError:
            data_formatada = str(dt_nascimento_medico) 
    medico = {
        "id_medico": id_medico,
        "nm_medico": nm_medico,
        "cpf_medico": str(cpf_medico),
        "dt_nascimento_medico": data_formatada,
                    ...
                    match opcao_paciente:
                        ...
                        case 2:
                            while True: 
                                ...
                                if verificar_cpf_main(NR_CPF, conn):
                                    break
                            id_paciente, nm_paciente, cpf_paciente, dt_nascimento_paciente, ddd, nr_telefone,  nm_logradouro, nr_logradouro, nm_cidade, sg_estado = ler_paciente(NR_CPF, conn)

                            #####sprint 4#####
                            resposta = deseja_salvar_json()
                            if resposta == True:
                                salvar_json_paciente(id_paciente, nm_paciente, cpf_paciente, dt_nascimento_paciente, ddd, nr_telefone,  nm_logradouro, nr_logradouro, nm_cidade, sg_estado)
                        case 3:
            ...
            case 2:
                ...
                while escolha_consulta.upper() == 'SIM':
                    ...
                    match opcao_consulta:
                        ...
                        case 2:
                            try: 
                                id_consulta, id_paciente_consulta, id_medico_consulta, data_consulta, descricao = ler_consulta(ID_CONSULTA, conn)

                                #####sprint 4#####
                                resposta = deseja_salvar_json()
                                if resposta == True:
                                    salvar_json_consulta(id_consulta, id_paciente_consulta, id_medico_consulta, data_consulta, descricao)
                            except ValueError as e:
                                print(f'O Id deve ser um número" \n{e}')
                        case 3:
                ...
                while escolha_medico.upper() == 'SIM':
                    ...
                    match opcao_medico:
                        ...
                        case 2:
                            try: 
                                ID_MEDICO = int(input('ID do médico: '))
                                id_medico, nm_medico, cpf_medico, dt_nascimento_medico, especialidade, crm = ler_medico(ID_MEDICO, conn)
                                #####sprint 4#####
                                resposta = deseja_salvar_json()
                                if resposta == True:
                                    salvar_json_medico(id_medico, nm_medico, cpf_medico, dt_nascimento_medico, especialidade, crm)
